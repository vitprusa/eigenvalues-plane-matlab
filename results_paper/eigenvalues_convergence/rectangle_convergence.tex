...
\begin{figure}[htbp]
  ...
  \caption{Eigenvalues $\lambda_1$, $\lambda_{100}$ and $\lambda_{1000}$ of Laplace operator, zero Dirichlet boundary conditions, rectangular domain \texttt{rectangle}, $(0, 2 \pi) \times (0, \pi)$. Eigenvalues computed by each discretisation \texttt{DST}, \texttt{FD} and \texttt{FEM} with varying degrees of freedom; compared with the exact eigenvalues $\lambda_k^{\text{exact}}$, the values of $\frac{m^2}{4} + n^2$, $m, n \geq 1$, in ascending order. Resolutions carrying fewer degrees of freedom than the index of the eigenvalue cannot supply it and are left out.}
