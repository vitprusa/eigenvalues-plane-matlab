% Convergence of the hundredth eigenvalue against the degrees of freedom, GWW1 isospectral drum:
...
\begin{figure}[htbp]
  \centering
  \subfloat[Eigenvalue $\lambda_{100}$.\label{fig:convergence_gww1_lambda100_value}]{%
    \includegraphics[width=0.45\textwidth]{gww1_convergence_lambda100}}
  \qquad
  \subfloat[Relative error $\frac{|\lambda_{100} - \lambda_{100}^{\text{DST}}|}{\lambda_{100}^{\text{DST}}}$.\label{fig:convergence_gww1_lambda100_error}]{%
    \includegraphics[width=0.45\textwidth]{gww1_convergence_lambda100_error}}
  \caption{The hundredth eigenvalue $\lambda_{100}$ of Laplace operator, zero Dirichlet boundary conditions, GWW1 isospectral drum \texttt{gww1}. Eigenvalue $\lambda_{100}$ computed by each discretisation \texttt{DST}, \texttt{FD} and \texttt{FEM} with varying degrees of freedom; compared with the reference eigenvalue $\lambda_{100}^{\text{DST}}$ computed by \texttt{DST} with 19875 degrees of freedom. Resolutions carrying fewer than 100 degrees of freedom cannot supply the eigenvalue and are left out.}
  \label{fig:convergence_gww1_lambda100}
\end{figure}
