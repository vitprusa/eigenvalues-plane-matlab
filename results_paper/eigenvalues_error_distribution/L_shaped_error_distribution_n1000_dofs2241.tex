...
% one series of markers per discretisation. L-shaped domain, linear index.
%
% The companion snippet L_shaped_error_distribution_n1000_dofs2241_log_log.tex
% holds the same data with a logarithmic index. Neither shows what the other
% does: the linear axis gives the body of the block the room it deserves, most of
% the thousand eigenvalues living in the last decade, and it is there that FD and
% FEM separate and rejoin; the logarithmic one opens out the first few indices,
% where DST is at its worst.
%
...
% thousand, returned to about 1e-12 relative to the closed form itself. Drawn
% together with the rest they swamp them, so they are the filled dots and the
% remaining indices the open circles. FD and FEM have no such subspace and are
% drawn whole.
%
...
% state. What it is worth is not uniform across the figure: at low index run and
% reference are both DST and share whatever the re-entrant corner does to them,
% so the DST markers there are an upper bound on its accuracy rather than a
% measured error. The FD and FEM series, orders of magnitude larger over most of
% the block, are unaffected.
%
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.85\textwidth]{L_shaped_error_distribution_n1000_dofs2241}
  \caption{Accuracy of the first 1000 eigenvalues of Laplace operator, zero Dirichlet boundary conditions, L-shaped domain \texttt{L\_shaped}. Relative error $\frac{|\lambda_k - \lambda_k^{\text{DST}}|}{\lambda_k^{\text{DST}}}$ of each eigenvalue against its index $k$, $k = 1, \dots, 1000$, for three discretisations of Laplace operator at comparable numbers of degrees of freedom -- \texttt{DST} and \texttt{FD} 2241, \texttt{FEM} 2257 -- compared with the reference eigenvalues computed by \texttt{DST} with 19845 degrees of freedom; \texttt{FEM} -- finite element method, \texttt{FD} -- finite differences method, \texttt{DST} -- discrete sine transform method. The \texttt{DST} series is split into the eigenvalues $\lambda_k = \pi^2(m^2+n^2)$ of the three unit squares, drawn as filled dots, and the remaining indices, drawn as open circles.}
  \label{fig:error_distribution_L_shaped}
\end{figure}
