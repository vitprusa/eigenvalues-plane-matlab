...
%WRITE_LATEX Write both tables (values and ordering) as one subfig LaTeX snippet.
    ...
    % Each subfloat carries its own label, so that either panel can be referred
    % to on its own, next to the label of the float as a whole below.
    fprintf(fid, ['  \\subfloat[\\label{tab:rectangle_indexing_M%d_a} ', ...
        'Computed eigenvalues in \\textbf{bold}. The ', ...
        'computational grid resolves $M_x = %d$ modes in the horizontal ', ...
        'direction $x$ and $M_y = %d$ modes in the vertical direction $y$, ', ...
        'total number of computed eigenvalues is $\\mathtt{DOF} = M_x M_y$. ', ...
        'Computed eigenvalues match---up to round-off error---the ', ...
        'corresponding analytic eigenvalues $\\lambda_{m,n}^{\\mathtt{exact}}$, ', ...
        'which are computed for all pairs $(m, n)$ and are shown in normal ', ...
        'weight.]{%%\n'], ...
        meta.M, meta.Mx, meta.My);
    write_tabular(fid, lambda_tab, matched, mm, nn, colspec, @(v) sprintf('%.2f', v));
    fprintf(fid, '\n  }\n');
    fprintf(fid, '  \\\\\n');

    % Sub-table (b): the positions.
    fprintf(fid, ['  \\subfloat[\\label{tab:rectangle_indexing_M%d_b} ', ...
        'Sequential position of analytical eigenvalues ', ...
        '$\\lambda_{m,n}^{\\mathtt{exact}}$ in the sort-by-size sequence of ', ...
        'all analytic eigenvalues.]{%%\n'], meta.M);
    write_tabular(fid, rank_tab, matched, mm, nn, colspec, @(v) sprintf('%d', v));
    fprintf(fid, '\n  }\n');

    fprintf(fid, ['  \\caption{Eigenvalues of Laplace operator, zero ', ...
        'Dirichlet boundary conditions, rectangular domain ', ...
        '\\texttt{rectangle}. The analytical eigenvalues of the ', ...
        'continuous eigenvalue problem are given by the formula~', ...
        '$\\lambda_{m,n}^{\\mathtt{exact}} = \\frac{m^2}{4} + n^2$. ', ...
        '(Eigenvalues indexed by~$(m, n)$, rows are indexed by~$m$, columns ', ...
        'by~$n$.) Numerical eigenvalues computed using the \\texttt{DST} ', ...
        'based discretisation, $M = %d$, $\\mathtt{DOF} = %d$.}\n'], ...
        meta.M, meta.dofs);
...
%   Matched cells are wrapped in \textbf{...}. fmt maps a value to its string.
    % \scriptsize scopes to the tabular (grouped) so the subcaptions stay normal
    % size.
