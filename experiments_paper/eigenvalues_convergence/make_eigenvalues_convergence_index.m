...
%   caveats in PLOT_ERROR_RUNS.
    ...
    % -name filter), caption name, DST/FD resolutions, FEM mesh sizes, published
    % lambda_1, and the resolution of the reference run (see DST_REFERENCE).
    extra = { ...
...
%   figures of different indices can be read against each other.
    ...
    if ~isempty(cfg.ref_index) && index == cfg.ref_index
        ref = struct('value', cfg.ref_value, 'label', cfg.ref_label, ...
                     'legend', sprintf('$\\mathtt{%s}$', cfg.ref_label), ...
                     'published', true, 'relative', true, 'dst_trim', 0);
...
%   are drawn whole either way.
    ...
    if ref.relative
        ...
        ylabel(ax, sprintf(['$\\frac{|\\lambda_{%d} - \\lambda_{%d}^{\\mathtt{%s}}|}', ...
                            '{\\lambda_{%d}^{\\mathtt{%s}}}$'], ...
                           index, index, ref.label, index, ref.label), 'FontSize', 16);
    else
        ylabel(ax, sprintf('$|\\lambda_{%d} - \\lambda_{%d}^{\\mathtt{%s}}|$', ...
