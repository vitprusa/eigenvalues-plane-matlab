% DST_LAPLACE_PARTIAL_SPECTRUM_L_SHAPED First few Dirichlet-Laplacian eigenvalues on the L-shaped domain.
%
%   Computes only the first few eigenvalues of the discrete Laplace operator
%   with homogeneous Dirichlet boundary conditions on the L-shaped domain. The
...
% M+1 is even so the re-entrant corner at the origin falls on a grid line.
M = 299;
...
phi = @(x,y) indicator_L_shaped(x, y, -1, 0, 1, -1, 0, 1);   % L-shaped domain
