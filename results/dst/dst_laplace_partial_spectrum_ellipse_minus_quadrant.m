% DST_LAPLACE_PARTIAL_SPECTRUM_ELLIPSE_MINUS_QUADRANT First few Dirichlet-Laplacian eigenvalues on the ellipse minus a quadrant.
%
%   Computes only the first few eigenvalues of the discrete Laplace operator
%   with homogeneous Dirichlet boundary conditions on the ellipse-minus-a-
%   quadrant domain. The domain is described by an indicator function embedded
%   in a rectangular bounding box and discretised on a uniform grid; the
%   DST-based Laplace operator is built with make_dst_laplace_op_batched and
%   the leading eigenvalues are obtained with the iterative solver eigs.
%
...
phi = @(x,y) indicator_ellipse_minus_quadrant(x, y, -2, 0, 2, 0);   % ellipse minus a quadrant
