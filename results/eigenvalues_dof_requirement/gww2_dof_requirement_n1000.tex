...
% sweep as dotted continuations. GWW2 isospectral drum.
%
% The domain has no closed-form spectrum, so the reference is a DST run of its
% own at M = 227, 19875 degrees of freedom, finer than every run of the sweep.
...
%
% Note that each drum is referred to its own DST run, not to a shared one: the
% two are isospectral in the continuum, but a reference computed on one geometry
% would import that geometry's discretisation error into the other's table.
% Keeping them separate is what lets the two answer tables be compared at all.
%
% This drum and \texttt{gww1} are isospectral -- the Gordon-Webb-Wolpert pair,
% the standard answer to Kac's question -- so the two figures are of the same
% spectrum reached through two different geometries, and the companion snippet
% gww1_dof_requirement_n1000.tex is the check on this one. What may differ
% between them is only how hard each geometry is to discretise, not what is
% being computed.
%
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.85\textwidth]{gww2_dof_requirement_n1000}
  \caption{Accuracy of the first 1000 eigenvalues of Laplace operator, zero Dirichlet boundary conditions, GWW2 isospectral drum domain \texttt{gww2}. Largest relative error over the whole block of first $1000$ eigenvalues, $\left\{ \lambda_k \right\}_{k=1}^{1000}$, $\max_{k \leq 1000} \frac{|\lambda_k - \lambda_k^{\text{DST}}|}{\lambda_k^{\text{DST}}}$, against the degrees of freedom used by each discretisation of Laplace operator, compared with the reference eigenvalues computed by \texttt{DST} with 19875 degrees of freedom. Each marker is labelled with the computation time of the particular run, dotted lines show extrapolated values.}
  \label{fig:dof_requirement_gww2}
\end{figure}
