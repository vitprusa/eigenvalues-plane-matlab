% DST_LAPLACE_PARTIAL_SPECTRUM_ISOSCELES_TRIANGLE First few Dirichlet-Laplacian eigenvalues on the isosceles right triangle.
%
%   Computes only the first few eigenvalues of the discrete Laplace operator
%   with homogeneous Dirichlet boundary conditions on the isosceles right
%   triangle. The domain is described by an indicator function embedded in a
...
phi = @(x,y) indicator_isosceles_triangle(x, y, 0, pi, 0);   % isosceles right triangle
